...
function generaAforoHTML() {
    ...
    const aforoHTML = asientos
        .map((asiento, indice) => {
            var clase = valorClassDeAsientoLIBRE;
            switch (asiento) {
                case LIBRE:
...
function ___actualizaTipoDeAsiento(asientoHTML) {
    ...
    console.log(asientoHTML);
    try {
        var indice = asientoHTML.attributes.getNamedItem('data-index').value;
    } catch (err) {
        console.log(err);
    }
    //al dar click accidentalmente en el espacio entre los asientos se genera un error que no permite 
    //seguir seleccionando asientos, usamos try-catch para recoger este error
    // y hacer que finalice esta funcion para poder seguir usando la páginas
    indice = Number(indice);

    let estado = asientos[indice];
    ...
    if (estado == LIBRE) {
        ...
        const selector = `[data-index="${indice}"]`;
        const asientoDOM = aforo.querySelector(selector);
        asientoDOM.classList = valorClassDeAsientoSELECCIONADO;
...
function ___recuperaAsientosDelAlmacenLocal() {
    ...
    if (asientos != null) {
        asientos = asientos.split(',').map(a => parseInt(a));
