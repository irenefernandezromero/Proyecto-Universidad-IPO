...
const nombreDelEquipoDeLaboratorio = "ITALIA";
...
function ___detenerTestDeVelocidad() {
  ...
  //Detenemos el temporizador usando la función clearInterval, que lo que hace es borrar el intervalo que habíamos definido con setInterval
  identificadorTemporizador = clearInterval(identificadorTemporizador);  
  //Posteriormente actualizo el valor de la variable a 0 (De esta forma, se seguirá marcando el número de segundos una vez lo detengamos)
...
function ___comprobarPalabraMecanografiada(palabraEscritaEnPanelDeEntrada) {
  ...
  if(palabraEscritaEnPanelDeEntrada == palabraDeMuestra.innerText){  //Compruebo si las palabras coinciden
    palabrasCorrectas.innerText++;  //En caso de sí hacerlo incremento el valor de las palabras correctas con ++
    actualizaPalabraDeMuestra();  //Actualizo el valor de la palabra llamando a la función actualizaPalabraDeMuestra()
    entrada.value = "";  //Vacío el panel de entrada poniendo su valor a ""
  }
}